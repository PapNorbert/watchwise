...
export default function WatchGroupsAll() {
  ...
  useEffect(() => {
    ...
    if (parseInt(limit) != limit) {
      ...
      let newUrl = `/api/watch_groups/?page=${page}&limit=${limit}`;
  ...
  }, [creatorSearch, limit, nameSearch, page, setLimit, setPage, setSortBy,
    locationSearch, showSearch, sortBy, userLocation, watchDateSearch, onlyNotFullGroupsSearch,
    watch_groups?.pagination.totalPages, maxDistanceSearch, locationAvailable]
  )
